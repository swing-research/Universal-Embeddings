...
#######################################
# Save
model_name = "Sphere_dimension_Euclidean" # results will be saved in results/model_name
...
for Ndim in Ndim_list:
    ...
    ### Define network
    #######################################
    net_Euclidean = models.NetMLP(inDim, outDim, N_latent=Nlatent, p=0.2, bn=False).to(device).train()
    ...
    if train:
        ...
        for ep in range(epochs):
            ...
            if ep%Ntest==0 and ep!=0:
                ...
                out = net_Euclidean(input)
                dist_mat_test = utils.distance_matrix(out)
        ...
        for num_rep in range(Nrep):
            ...
            # net_Euclidean = DirectOptimization1D_NN_v2(inDim, outDim, N_latent=128, weights=True).to(device).train()
